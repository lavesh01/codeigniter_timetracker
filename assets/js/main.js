const resetValues = () => {
  ...
  const createTaskComponent = () => {
    ...
    resetValues();
    saveTaskComponents(); // Save task components to database
  }
  ...
  const saveTaskComponents = () => {

    const url = 'http://localhost/timetracker/index.php/timetracker/save_task_components';
    const data = {
    ...
    xhr.onload = () => {
      if (xhr.status === 200) {
        console.log('Task components saved successfully');
      } else {
        console.log('Error saving task components:', xhr.statusText);
      }
    };
    xhr.onerror = () => {
      console.log('Error saving task components:', xhr.statusText);
